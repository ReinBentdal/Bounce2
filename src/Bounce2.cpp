...
bool Debouncer::update() {
  ...
  if (readState != getStateFlag(DEBOUNCED_STATE)) {
    ...
    if (millis() - previous_millis >= interval_millis) {
      ...
      // This will be prompt as long as there has been greater than interval_misllis ms since last
      // change of input. Otherwise debounced state will not change again until bouncing is stable
      // for the timeout period.
      changeState();
    }
  }

  // If the readState is different from previous readState, reset the debounce timer - as input is
  // still unstable and we want to prevent new button state changes until the previous one has
...
// WIP HELD
unsigned long Debouncer::previousDuration() const { return durationOfPreviousState; }

unsigned long Debouncer::currentDuration() const { return (millis() - stateChangeLastTime); }
...
bool Debouncer::read() const { return getStateFlag(DEBOUNCED_STATE); }
